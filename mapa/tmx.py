...
class Tile(object):
	def __init__(self, gid, surface, tileset):
		self.gid = gid
		# print "Este es el gid ", self.gid
		self.surface = surface
		# print "Este es el surface ",self.surface
		self.tile_width = tileset.tile_width
		# print "Este es el tile_widht ",self.tile_width
		self.tile_height = tileset.tile_height
		# print "Este es el tile_height ",self.tile_height
		self.properties = {}
		# print "Estas son las properties",self.properties
	...
	def loadxml(self, tag):
		...
		for c in props.findall('property'):
			...
			# print "c.attrib name",c.attrib['name']
			# print "c.attrib value",c.attrib['value']
			name = c.attrib['name']
...
class Tileset(object):
	def __init__(self, name, tile_width, tile_height, firstgid):
		...
		# print "self.firstgid", self.firstgid
		self.tiles = []
		self.properties = {}
	...
	def fromxml(cls, tag, firstgid=None):
		...
		for c in tag.getchildren():
			if c.tag == "image":
				...
				# print "aqui manda el add_image",c.attrib['source']
			elif c.tag == 'tile':
...
class Tilesets(dict):
	def add(self, tileset):
		for i, tile in enumerate(tileset.tiles):
			i += tileset.firstgid
			self[i] = tile
			# print "en class TIlesets",self[i]
...
class Cell(object):
	...
	def __repr__(self):
		print '<Cell %s,%s %d>' % (self.px, self.py, self.tile.gid)
...
class Layer(object):
	...
	def collide(self, rect, propname):
		...
		for cell in self.get_in_region(rect.left, rect.top, rect.right,
			...
			if propname in cell:
				r.append(cell)
		return r
	#Metodo que verifica si en la celda que esta sobre el jugador se encuentra un elemento con la propiedad propname
	def whatsup(self, rect, orientacion, propname):
		'''Find all cells the rect is touching that have the indicated property
		name set.
		'''
		r = []
		for cell in self.get_in_region(rect.left, rect.top - 32 , rect.right,
				rect.bottom):
			if propname in cell and orientacion == 'up':
				r.append(cell)
		for cell in self.get_in_region(rect.left - 32, rect.top  , rect.right,
				rect.bottom):
			if propname in cell and orientacion == 'left':
				r.append(cell)
		for cell in self.get_in_region(rect.left , rect.top  , rect.right + 32,
				rect.bottom):
			if propname in cell and orientacion == 'right':
				r.append(cell)
		for cell in self.get_in_region(rect.left , rect.top  , rect.right,
				rect.bottom + 32):
			if propname in cell and orientacion == 'down':
				r.append(cell)


		return r
	#Metodo que verifica si en la celda que esta a la izq del jugador se encuentra un elemento con la propiedad propname
	def whatsleft(self, rect, propname):
		'''Find all cells the rect is touching that have the indicated property
		name set.
		'''
		r = []
		for cell in self.get_in_region(rect.left - 32, rect.top  , rect.right,
			...
			if propname in cell:
				r.append(cell)
		return r
	#Metodo que verifica si en la celda que esta a la der del jugador se encuentra un elemento con la propiedad propname
	def whatsright(self, rect, propname):
		'''Find all cells the rect is touching that have the indicated property
		name set.
		'''
		r = []
		for cell in self.get_in_region(rect.left, rect.top, rect.right + 32,
			...
			if propname in cell:
				r.append(cell)
		return r
	#Metodo que verifica si en la celda que esta abajo del jugador se encuentra un elemento con la propiedad propname
	def whatsdown(self, rect, propname):
		'''Find all cells the rect is touching that have the indicated property
		name set.
		'''
		r = []
	...
	def get_cell_at(self, x, y):
		...
		i = x // self.tile_width
		j = y // self.tile_height
		return r.append(self.cells.get((i, j)))
